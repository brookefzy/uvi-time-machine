...
def get_confidence(df, conf):
    ...
    df_con['object_name'] = df_con['object_name'].apply(lambda x: x.lower().replace("adult_", ""))
    ...
    df_summary = df_con.groupby(['img','object_name']).size().reset_index().pivot(
        index='img', columns = ['object_name'], values=0
    ).fillna(0)
...
def get_result(city):
    ...
    for res in [8,9]:
        ...
        for conf in [0.2, 0.5, 0.7, 0.8]:
            ...
            standard_cols = ['male', 'female','age_18','age_60','pedestrian_count']
    ...
    alldf.to_parquet(FILE_TO_EXPORT.format(ROOTFOLDER=ROOTFOLDER, 
                                           cityabbr=cityabbr, 
                                           version = VERSION), index=False)
...
#python3 /home/yuanzf/uvi-time-machine/_script/e-street-diversity/02c_age_gender_summary-4label.py