...
def get_hex_basics(city_abbr):
    ...
    # keep these hex
    df_all_keep = df_pano_inner[
        (df_pano_inner["year"] != year_to_exclude) & (df_pano_inner["year"] >= 2015)
    ].reset_index(drop=True)
    print(df_all_keep.shape[0], "panoids are kept")
    df_all_keep["year_group"] = df_all_keep["year"].apply(
        lambda x: YEAR_GROUP[0] if x < 2019 else YEAR_GROUP[1]
    )

    # get number of panoid per hex
    df_all_keep_hex = (
        df_all_keep.groupby(["hex_id", "year_group"])["panoid"]
        .nunique()
        .reset_index(name="panoid_n")
    )
    df_all_keep_hex = (
        df_all_keep_hex[df_all_keep_hex["panoid_n"] >= min_num_pano]
        .groupby("hex_id")
        .filter(lambda x: len(x) == 2)
        .reset_index(drop=True)
    )
    df_all_keep_hex["geometry"] = df_all_keep_hex["hex_id"].apply(
        lambda x: shapely.geometry.Polygon(h3.h3_to_geo_boundary(x, geo_json=True))
    )
...
def summarize_objects(city, viz=False):
    city_abbr = city.lower().replace(" ", "")
    df_all_keep_hex, df_all_keep = get_hex_basics(city_abbr)
    ...
    object_summary = (
        df_all_keep[["panoid", "year_group", "hex_id"]]
        .merge(df_sel, on="panoid", how="left")
        .groupby(["hex_id", "year_group", "object_name"])["n"]
        .sum()
        .reset_index()
        .merge(
            df_all_keep_hex[["hex_id", "panoid_n", "year_group"]],
            on=["hex_id", "year_group"],
            how="right",
        )
        .fillna(0)
    ...
    df_all_keep_hex[["hex_id", "year_group"]].to_csv(
        f"{EXFOLDER_LONG}/c_hex_{city_abbr}.csv", index=False
    )
