...
def get_seg_data(city, hex_detail_cluster):
    ...
    seg_df_pivot_sel_stack_scaled_keys = seg_df_pivot_sel_stack[[ 'img', 'year', 'h3_9','cluster_8']]
    seg_df_pivot_sel_stack = pd.concat([seg_df_pivot_sel_stack_scaled_keys, pd.DataFrame(seg_df_pivot_sel_stack_scaled, columns = feature_ls)], axis = 1)
...
hex_detail_cluster = hex_detail.merge(hex_df[['hex_id', 'cluster_8','city_lower']], on = 'hex_id')
...
cluster_mean = hex_detail_cluster[feature_ls+['cluster_8']].groupby('cluster_8').mean()
...
for city in CITY_LS[:]:
    ...
    for cluster_i in range(8):
        sel_cluster_sample(city, cluster_mean, seg_df_pivot_sel_stack, meta_df, cluster_i)
