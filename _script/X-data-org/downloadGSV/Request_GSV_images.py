...
def load_data(args):
    ...
    imgGDF = pd.read_csv(os.path.join(dirsave, filename))
    ...
    def load_finish_from_file(dirsave):
        finished = pd.read_csv(os.path.join(dirsave, "gsv_path.csv"))
        return finished['panoid'].unique()
