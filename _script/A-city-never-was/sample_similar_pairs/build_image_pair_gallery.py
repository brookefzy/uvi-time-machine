...
def _render_html(
    ...
    *,
    ...
    escaped_similarity_label = html.escape(similarity_label)
    for row in rows:
        ...
        cards.append(f'''<article class="pair"><h2>{city_1} ↔ {city_2}</h2><p>{escaped_similarity_label}: {float(row["cosine_similarity"]):.4f}</p><div class="columns"><section><h3>{city_1}</h3><img src="images/{image_1}" alt="{city_1} sample"><p>{html.escape(str(row["panoid_1"]))}</p><div id="map-{identifier}-1" class="map"></div></section><section><h3>{city_2}</h3><img src="images/{image_2}" alt="{city_2} sample"><p>{html.escape(str(row["panoid_2"]))}</p><div id="map-{identifier}-2" class="map"></div></section></div></article>''')
        maps.append(
...
def build_gallery(
    ...
    *,
    ...
    (output_dir / "index.html").write_text(
        _render_html(manifest_rows, title=title, similarity_label=similarity_label)
    )
    return manifest
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--title", default="DINOv3 cross-city similar-image samples")
    parser.add_argument("--similarity-label", default="Cosine similarity")
    return parser
