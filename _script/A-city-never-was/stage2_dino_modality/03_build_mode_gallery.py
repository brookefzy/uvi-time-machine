...
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument("--representatives",type=Path,required=True);p.add_argument("--output",type=Path,required=True);a=p.parse_args();render_gallery(pd.read_parquet(a.representatives),a.output)
