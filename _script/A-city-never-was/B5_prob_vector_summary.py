...
TRAIN_TEST_FOLDER = (
    "/lustre1/g/geog_pyloo/05_timemachine/_transformed/t_classifier_img_yolo8"
)
RAW_PATH = (
...
def load_source(city):
    ...
    for res in [6, 7, 8, 11, 12, 13]:
        try:
            ...
            print(
                "version {} does not support, geo_to_h3, use latlng_to_cell instead".format(
                    h3.__version__
                )
            )
            df_pano_inner[f"hex_{res}"] = df_pano_inner.apply(
...
def prepare_data(city):
    ...
    temp = pd.concat(
        pd.read_parquet(result_sel.file_path.values[i])
        for i in range(len(result_sel.file_path.values))
    )
    temp = temp.drop_duplicates(subset=["name"]).reset_index(drop=True)
    temp["panoid"] = temp["name"].apply(lambda x: x[:22])

    # get all train and test image names
    train_test = glob(
        os.path.join(TRAIN_TEST_FOLDER, f"*/{city}/*.jpg"), recursive=True
    )
    ...
    train_test_merge = df_pano_inner[
        df_pano_inner["panoid"].isin(train_test_panoid)
    ].reset_index(drop=True)
...
def robust_exclusion(df, train_test_merge, res_exclude=None):
    ...
    if res_exclude is None:
        ...
        print(
            "Exclude validation images if they share the same hex as training images at resolution: ",
            res_exclude,
        )
        df_clean = df[
            ~df[f"hex_{res_exclude}"].isin(train_test_merge[f"hex_{res_exclude}"])
        ].reset_index(drop=True)
    resulthex = []
    for res in [6, 7, 8]:

        summary = (
            df_clean[vector_ls + [f"hex_{res}"]]
            .groupby(f"hex_{res}")
            .mean()
            .reset_index()
        )
...
def get_viz(resulthex, res=8):
    ...
    viz["second_col"] = viz[vector_ls].apply(
        lambda x: x.drop(viz["max_col"]).idxmax(), axis=1
    )
...
def get_result(city):
    ...
    for res_exclude in [None, 11, 12, 13]:
        ...
        result.to_parquet(
            os.path.join(
                CURATE_FOLDER_EXPORT,
                OUTPUT_FILE_NAME.format(city=city, res_exclude=str(res_exclude)),
            )
        )
...
# %%
# CITY_TO_OVERWRITE = ['Santiago', ]
CITY_TO_OVERWRITE = ["Santiago", "Minneapolis"]
...
def main():
    ...
    finished_city_ls = set(
        [x.split("/")[-1].split("_")[1].replace("city=", "") for x in finished]
    )
    finished_city_ls = [x for x in finished_city_ls if x not in CITY_TO_OVERWRITE]
    ...
    if city == "all":
        ...
        for city in tqdm(city_ls):
            ...
            finished = glob(
                os.path.join(CURATE_FOLDER_EXPORT, "*.parquet"), recursive=True
            )
            finished_city_ls = set(
                [x.split("/")[-1].split("_")[1].replace("city=", "") for x in finished]
            )
            finished_city_ls = [
                x for x in finished_city_ls if x not in CITY_TO_OVERWRITE
            ]
