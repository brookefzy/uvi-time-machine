#!/usr/bin/env bash
#SBATCH --job-name=classifier_img_pairs
...
"${PYTHON}" "${REPO_DIR}/sample_similar_pairs/sample_classifier_image_pairs_faiss.py" \
  ...
  --min-year "${MIN_YEAR:-2012}" \
  --max-year "${MAX_YEAR:-2022}" \
  ...
  --top-k "${TOP_K:-30}" \
  --threshold "${CLASSIFIER_THRESHOLD:--1.0}" \
  ...
  --output "${OUTPUT:-sample_similar_pairs/output/classifier_image_pairs.parquet}"
