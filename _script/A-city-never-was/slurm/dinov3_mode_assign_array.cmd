#!/usr/bin/env bash
#SBATCH --job-name=dinov3_mode_assign
...
read -r SELECTED_K MODEL_ID <<< "$("${PYTHON}" -c 'import json,sys; x=json.load(open(sys.argv[1])); print(x["selected_k"], x["model_id"])' "${SELECTED_MODEL}")"
"${PYTHON}" "${REPO_DIR}/stage2_dino_modality/05_assign_images_to_modes.py" --input "${MODE_OUTPUT_ROOT}/sampled_images/city=${CITY}.parquet" --centroids "${MODE_OUTPUT_ROOT}/codebook_candidates/k=${SELECTED_K}/centroids.parquet" --selected-model "${SELECTED_MODEL}" --output "${MODE_OUTPUT_ROOT}/model=${MODEL_ID}/assignments/city=${CITY}.parquet"
