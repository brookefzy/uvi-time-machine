...
PAIR_INDEX_OFFSET="${PAIR_INDEX_OFFSET:-0}"
[[ "${PAIR_INDEX_OFFSET}" =~ ^[0-9]+$ ]] || { printf 'PAIR_INDEX_OFFSET must be a non-negative integer: %s\n' "${PAIR_INDEX_OFFSET}" >&2; exit 2; }
[[ "${SLURM_ARRAY_TASK_ID:?Set SLURM_ARRAY_TASK_ID}" =~ ^[1-9][0-9]*$ ]] || { printf 'SLURM_ARRAY_TASK_ID must be a positive integer: %s\n' "${SLURM_ARRAY_TASK_ID}" >&2; exit 2; }
PAIR_INDEX=$((PAIR_INDEX_OFFSET + SLURM_ARRAY_TASK_ID))
PAIR="$("${PYTHON}" -c 'import sys; rows=open(sys.argv[1]).read().splitlines(); index=int(sys.argv[2]); assert 1 <= index <= len(rows), f"pair index {index} outside manifest range 1..{len(rows)}"; print(rows[index-1])' "${PAIR_MANIFEST}" "${PAIR_INDEX}")"; IFS='|' read -r CITY_1 CITY_2 <<< "${PAIR}"
[[ -n "${CITY_1}" && -n "${CITY_2}" && "${CITY_1}" != "${CITY_2}" ]] || { printf 'Invalid city pair: %s\n' "${PAIR}" >&2; exit 2; }
OUTPUT="${MODE_OUTPUT_ROOT}/model=${MODEL_ID}/h3_similarity/city_1=${CITY_1}/city_2=${CITY_2}/part_res=8.parquet"
if [[ "${RESUME:-1}" == "1" && -f "${OUTPUT}" ]]; then
  printf 'Reusing existing similarity shard for pair %s: %s\n' "${PAIR_INDEX}" "${OUTPUT}"
  exit 0
fi
"${PYTHON}" "${REPO_DIR}/stage2_dino_modality/07_compute_h3_mode_js_similarity.py" --source "${MODE_OUTPUT_ROOT}/model=${MODEL_ID}/h3_histograms/city=${CITY_1}.parquet" --target "${MODE_OUTPUT_ROOT}/model=${MODEL_ID}/h3_histograms/city=${CITY_2}.parquet" --output "${OUTPUT}" --threshold "${SIMILARITY_THRESHOLD:--1}" --row-block-size "${SIMILARITY_ROW_BLOCK_SIZE:-64}" --target-block-size "${SIMILARITY_TARGET_BLOCK_SIZE:-2048}"
