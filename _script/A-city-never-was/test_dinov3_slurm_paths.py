...
def test_b5c_job_passes_shared_pairwise_and_resolution_specific_export_roots() -> None:
    job = (REPO_ROOT / "slurm" / "dinov3_04_b5c_aggregate.cmd").read_text(encoding="utf-8")

    assert 'RESOLUTION="${RESOLUTION:-7}"' in job
    assert 'PAIRWISE_ROOT="${PAIRWISE_ROOT:-${ROOTFOLDER}/_curated/c_city_dinov3_similarity_by_pair}"' in job
